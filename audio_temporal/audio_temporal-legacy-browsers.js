...
let expName = 'audio_temporal';  // from the Builder filename that created this script
...
function experimentInit() {
  ...
  welcomeText = new visual.TextStim({
    win: psychoJS.window,
    name: 'welcomeText',
    text: 'In this experiment, you will hear two pairs of AUDITORY stimuli presented one after the other. \n\nYou will be asked to indicate whether the 2nd pair of sounds was separated by a duration that was shorter or longer than the first pair of sounds.  Please make your decision using the Left or Right Arrow keys. \n\nPress Right Arrow to start experiment.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0,
    color: new util.Color('white'),  opacity: 1,
  ...
  instruction_3 = new visual.TextStim({
    win: psychoJS.window,
    name: 'instruction_3',
    text: '\nWas the 2nd pair of sounds separated by a shorter or longer duration than the 1st pair?\n\nshorter=Left Arrow  longer=Right Arrow',
    font: 'Arial',
    units: undefined, 
...
function remindersRoutineBegin(snapshot) {
  return function () {
    ...
    remindersComponents.forEach( function(thisComponent) {
...
function startSessionRoutineBegin(snapshot) {
  return function () {
    ...
    startSessionComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function prepareBlockRoutineBegin(snapshot) {
  return function () {
    ...
    prepareBlockComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function prepareTrialRoutineBegin(snapshot) {
  return function () {
    ...
    wavArray1 = createWav(f_standard, 1.0, duration/1000);
    ...
    wavArray2 = createWav(f_standard, 1.0, duration/1000);
    ...
    prepareTrialComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function endSessionRoutineBegin(snapshot) {
  return function () {
    ...
    endSessionComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
